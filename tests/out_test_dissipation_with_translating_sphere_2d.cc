  ...
  for (unsigned int degree=1; degree<=max_degree; degree++)
    {
      ...
      std::string fe_name_stokes = "FESystem<2,3>[FE_DGQ<1,2>(0)^3]";
      std::string fe_name_map = "FESystem<2,3>[FE_Q<1,2>(1)^3]";
      post_process.fe_stokes = SP(FETools::get_fe_by_name<1,2> (fe_name_stokes));
      post_process.fe_map = SP(FETools::get_fe_by_name<1,2> (fe_name_map));
